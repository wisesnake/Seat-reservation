...
//            일련번호 대조하는 반복문
            boolean flag = false;
            for (int i = 1; i < main.seat.length; i++) {
              
            	if (checkbookedCustNo == main.seat[i].getBookedCustNo()) {
                    System.out.printf("일련번호는 %d이며, 예약하신 좌석은 %d번 좌석입니다.\n", main.seat[i].getBookedCustNo(), main.seat[i].getSeatNo());
